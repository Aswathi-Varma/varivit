...
sys.stdout = open('/root/sys_out_res', 'w')
...
from utils.metrics_multi import calculate_metrics
...
def get_class_weights(y):
    zeros_combined = np.count_nonzero(y == 0)
    ones_combined = np.count_nonzero(y == 1)

    class_counts = np.array([zeros_combined, ones_combined])
    total_samples = len(y)
    class_weights = torch.tensor([total_samples / (count * len(class_counts)) for count in class_counts], dtype=torch.float)
...
@torch.no_grad()
def evaluate(data_loader, model, device, weight, mode=None):
    ...
    mcc_score, f1_score, precision, recall, roc_auc_score = calculate_metrics(predictions=outPRED, target=outGT, mode=mode)

    metric_logger.update(mcc_score=mcc_score)
    ...
    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
    print(' * MCC {:.3f}, F1-score {:.3f}, Precision {:.3f}, Recall {:.3f}, roc_auc_score {:.3f}, loss {losses.global_avg:.3f}'
         .format(mcc_score, f1_score, precision, recall, roc_auc_score, losses=metric_logger.loss))
    #print('* roc_auc_score {:.3f}, f1_score {:.3f}, precision {:.3f} , recall {:.3f}, loss {losses.global_avg:.3f}'
     #     .format(roc_auc_score, f1_score, precision, recall, losses=metric_logger.loss))
...
def main(args):
    ...
    # Prepare dataloaders
    dataset_whole = get_dataset(dataset_name="glioma", mode="whole", args=args, transforms=train_transforms, use_z_score=args.use_z_score)
    dataset_whole_no_aug = get_dataset(dataset_name="glioma", mode="whole", args=args, transforms=None, use_z_score=args.use_z_score)

    #features, labels = get_all_feat_and_labels(dataset_whole, args=args)

    cross_entropy_wt = torch.as_tensor([2.5994, 0.4158, 4.7590]).to(device)
    ...
    split_index_path = os.path.join(PROJECT_ROOT_DIR, "glioma_base_multi", 'k_fold', 'indices_file')
    ...
    #for idx, (train_ids, test_ids) in enumerate(kfold_splits.split(features, labels)):
        # train_labels = np.array(labels)[train_ids]
        # train_ids, val_ids, _, _ = train_test_split(train_ids, train_labels, test_size=0.1, random_state=42, stratify=train_labels)
    for i in range(2):
        idx = i + 3
        ...
        model = generate_model(model_depth=18, n_classes=3, n_input_channels=4)
        model.to(device)
        ...
        max_mcc_score, max_roc_auc, max_f1_score = 0.0, 0.0, 0.0
        for epoch in range(args.start_epoch, args.epochs):
            ...
            train_val_stats = evaluate(dataloader_train, model, device, weight=cross_entropy_wt)
            val_stats = evaluate(dataloader_val, model, device, weight=cross_entropy_wt)

            print(f"MCC score of the network on the val images: {val_stats['mcc_score']:.1f}%")
            max_mcc_score = select_best_model(args=args, epoch=epoch, loss_scaler=loss_scaler,
                                                max_val=max_mcc_score,
                                                model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                                                cur_val=val_stats['mcc_score'],
                                                model_name=f'best_mcc_model_{idx}', output_dir=output_dir)
            # Let us save model based on the other criterions
            print(f"Micro-F1 score of the network on the val images: {val_stats['f1_score']:.1f}%")
            max_f1_score = select_best_model(args=args, epoch=epoch, loss_scaler=loss_scaler,
                                                max_val=max_f1_score,
                                                model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                                                cur_val=val_stats['f1_score'],
                                                model_name=f'best_f1_model_{idx}', output_dir=output_dir)

            print(f"ROC-AUC score of the network on the val images: {val_stats['roc_auc_score']:.1f}%")
            max_roc_auc = select_best_model(args=args, epoch=epoch, loss_scaler=loss_scaler,
                                                max_val=max_roc_auc,
                                                model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                                                cur_val=val_stats['roc_auc_score'],
                                                model_name=f'best_auc_model_{idx}', output_dir=output_dir)
            
            # Writing the logs
            log_writer_val.add_scalar('ft/roc_auc_score', val_stats['roc_auc_score'], epoch)
            log_writer_val.add_scalar('ft/mcc_score', val_stats['mcc_score'], epoch)
            log_writer_val.add_scalar('ft/f1_score', val_stats['f1_score'], epoch)
            ...
            log_writer_train.add_scalar('ft/mcc_score', train_val_stats['mcc_score'], epoch)
            log_writer_train.add_scalar('ft/f1_score', train_val_stats['f1_score'], epoch)
        ...
        args.finetune = os.path.join(output_dir, f"checkpoint-best_auc_model_{idx}.pth")
        ...
        test_stats  = evaluate(data_loader=dataloader_test, model=model, device=device, weight=None)
        ...
        global_mcc.append(test_stats['mcc_score'])
