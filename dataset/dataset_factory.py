...
def get_dataset(dataset_name, mode, args, sizes = None, transforms=None, use_z_score=False):
    ...
    if dataset_name == 'glioma':
        if sizes is None:
            return glioma.build_dataset(mode=mode, args=args, transforms=transforms, use_z_score=use_z_score)
        else:
