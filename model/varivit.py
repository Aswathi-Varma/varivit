#Model that interpolate the pos embedding from the max size to the current size
import copy
import math
...
class PatchEmbed3D(nn.Module):
    ...
    def __init__(self, max_volume_size=224, patch_size=16, in_chans=3, embed_dim=768, norm_layer=None, flatten=True):
        super().__init__()
        max_volume_size = traid(max_volume_size)
        patch_size = traid(patch_size)
        self.max_volume_size = max_volume_size
        self.patch_size = patch_size
        self.max_grid_size = (max_volume_size[0] // patch_size[0], max_volume_size[1] // patch_size[1], max_volume_size[2] // patch_size[2])
...
class VisionTransformer3D(nn.Module):
    ...
    def __init__(self, max_volume_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=384, depth=12,
        ...
        """
        Args:
            max_volume_size (int, triad): max input image size
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, max_num_patches + 1, embed_dim), requires_grad=False)  
    ...
    #interpolate:
    def interpolate_pos_embed(self, new_img_size, interpolation = "trilinear"):

        new_img_size = traid(new_img_size)
        
        new_size = (new_img_size[0] // self.patch_size[0], new_img_size[1] // self.patch_size[1], new_img_size[2] // self.patch_size[2])
        new_num_patches = new_size[0] * new_size[1] * new_size[1]

        old_size = self.patch_embed.max_grid_size
        ...
        if self.num_tokens:
            posemb_prefix, pos_embed = (
                self.pos_embed[:, :self.num_tokens],
                self.pos_embed[:, self.num_tokens:],
            )
        else:
            posemb_prefix, pos_embed = None, self.pos_embed

        # Interpolate position embedding
        
        pos_embed = pos_embed.reshape(1, old_size[0], old_size[1], old_size[2], -1).permute(0, 4, 1, 2, 3)
        pos_embed = F.interpolate(pos_embed, size=new_size, mode=interpolation)
        pos_embed = pos_embed.permute(0, 2, 3, 4, 1).reshape(1, new_num_patches, -1)
    ...
    #center:
    def centered_pos_embed(self, new_img_size):
        ...
        center = [old // 2 for old in old_size]
        start = [int(c - n // 2) for c, n in zip(center, new_size)]
        start = [max(0, s) for s in start]
        end = [s + n for s, n in zip(start, new_size)]

        # Slice pos_embed
        pos_embed_cropped = pos_embed[..., start[0]:end[0], start[1]:end[1], start[2]:end[2]]
        pos_embed = pos_embed_cropped.permute(0, 2, 3, 4, 1).reshape(1, new_num_patches, -1)
    ...
    def forward_features(self, x):
            ...
            outputs = []
            attention_maps = []  # Store attention maps from each block
            ...
            pos_embed = self.centered_pos_embed(new_img_size)
            x = x + pos_embed
            x = self.pos_drop(x)
