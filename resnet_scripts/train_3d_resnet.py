...
def get_all_feat_and_labels(dataset_whole, args):
    mri, labels = [], []
...
# def get_all_feat_and_labels(dataset_whole, args):
#     if args.in_channels == 1:
#         return torch.cat((x[0] for x in dataset_whole)), torch.stack(tuple(x[-1] for x in dataset_whole))
#     else:
#         return torch.stack(tuple(x[0] for x in dataset_whole)), torch.stack(tuple(x[-1] for x in dataset_whole))
...
def main(args, train=True): 
    ...
    if train:
        ...
        for idx in range(3):
            ...
            # Use train_ids and test_ids for the current fold
            ...
            model = generate_model(model_depth=18, n_classes=args.num_classes, n_input_channels=args.in_channels)
            ...
            print("-------------------------------NOTE---------------------------------------")
            sys.stdout.flush()
            cross_entropy_wt = torch.as_tensor([0.6029, 2.9286])
        ...
        print(f"ROC Score {avg_roc_score/3}, Specificity {avg_spec/3} and Sensitivity {avg_sen/3}")
        sys.stdout.flush()
