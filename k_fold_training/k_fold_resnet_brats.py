...
def get_args_parser():
    parser = argparse.ArgumentParser('Brats training', add_help=False)
...
def get_dataset_combined(modes, args, transforms):
    dataset = [get_dataset(dataset_name="brats", mode=mode, args=args, transforms=transforms, use_z_score=args.use_z_score) for mode in modes]
...
def main(args):
    ...
    #args initialization
    args = bootstrap(args=args, key='BRATS')
    ...
    # Prepare dataloaders
    dataset_whole = get_dataset(dataset_name="brats", mode="whole", args=args, transforms=train_transforms, use_z_score=args.use_z_score)
    dataset_whole_no_aug = get_dataset(dataset_name="brats", mode="whole", args=args, transforms=None, use_z_score=args.use_z_score)

    features, labels = get_all_feat_and_labels(dataset_whole, args=args)

    cross_entropy_wt = torch.as_tensor([0.6, 2.6]).to(device)
    ...
    split_index_path = os.path.join(PROJECT_ROOT_DIR, "brats_base", 'k_fold', 'indices_file')
    ...
    for idx, (train_ids, test_ids) in enumerate(kfold_splits.split(features, labels)):
        ...
        model = generate_model(model_depth=18, n_classes=2, n_input_channels=4)
        model.to(device)
...
def plot_ROCAUC(n_folds,probabilities,labels, roc_kfold):
    ...
    plt.savefig('roc_auc_brats_resnet.png')
